...
| 2026-06-11 | Groupes asynchrones | 9.2.1 à 9.2.9 | Modèles (Group/GroupMember/GroupBinder/GroupActivity), DAO, GroupService (règles : max 5 groupes créés / 20 rejoints, invite code 8 car. aléatoire), endpoints REST complets, hooks d'activité (joined/shared_binder/completed_session via StatsService). Frontend : store `groups.ts`, vues GroupsList/GroupDetail, entrée sidebar. |
| 2026-06-12 | Système professeur (Classes) | 10.2.1 à 10.2.9 | Group étendu (type/is_class), modèles Assignment/AssignmentProgress, ClassService (rôle teacher = owner/admin requis pour assigner, progression agrégée), endpoints classes + `/assignments/mine`, intégration des devoirs (deadline ≤ 3j) dans `FocusService.get_today_items`. Frontend : TeacherDashboard, StudentClassView, AssignmentDetail. |
| 2026-06-13 | Conformité Groupes & Classes | Audit FEATURES.md | Audit de conformité des Features 9 & 10 : modèles, endpoints, rôles, règles métier, hooks d'activité et confidentialité conformes. Ajout des tests nommés manquants `test_activity_recorded_on_join` et `test_member_cannot_access_other_member_flashcards` (isolation des flashcards entre membres vérifiée). |
| 2026-06-13 | Révision par évaluation IA d'une note | Feuille d'évaluation mixte | Refonte de la révision d'une note : feuille d'évaluation IA mixte (qcm/vf/trou/open) générée en un seul appel Gemini avec clé de correction embarquée + fusion des items issus des balises (corrigés sans IA). Cache par `content_hash`, bouclage SM-2 (items ratés → deck fantôme `source='ai'`), colonne `Flashcard.source`, modèles/DAO/service/schémas, tâche Celery async + endpoints `/evaluations`. Frontend : vue `NoteEvaluation`, onglet Reviews, bouton NoteEdit. Correctif binding Celery `ContextTask`. Tests backend (service/endpoints/SM-2) + vue-tsc/Vitest verts. |
| 2026-06-15 | Suppression des decks fantômes | Cartes d'éval opt-in | Plus aucune flashcard n'est créée automatiquement depuis une note : suppression du concept de deck fantôme (`Deck.note_id` + migration non destructive qui détache/renomme les decks existants). `note_service` ne synchronise plus de cartes depuis les balises ; `diagram_service` ne re-synchronise plus les notes ; `community_service` clone les decks normaux. La complétion d'une évaluation IA PROPOSE désormais des cartes pour les items ratés (`proposed_cards`), et l'élève choisit de les ajouter (ou non) à un deck réel via `POST /evaluations/:id/flashcards` (idempotent, appartenance vérifiée). Frontend : sélection + sélecteur de deck dans `NoteEvaluation`. Le mode « Révision Active » de `NoteEdit` (blocs de révision interactifs intégrés aux notes) est **conservé** ; comme la note ne porte plus de cartes, les boutons de notation SM-2 (qui notaient les cartes fantômes) ne s'affichent plus — l'interactivité (révéler/répondre) reste. Les balises de note alimentent toujours les évaluations IA. Tests backend + vue-tsc/Vitest verts. |
| 2026-06-15 | Classeur : dossier de révision thématique | Items typés + ajout unifié | Le classeur devient un vrai dossier de révision. Les jeux de révision (decks) acceptent désormais, en plus des cartes recto/verso, des items typés **QCM / Vrai-Faux / Ordre / Association** (`Flashcard.card_type` + `payload` JSON, migration additive), tous planifiés en répétition espacée SM-2. Backend : schémas + service + tests. Frontend : menu unifié **« + Ajouter »** dans `Binders.vue` (sous-dossier, note, jeu de révision, carte, QCM, VF, ordre, association), section « Jeux de révision » avec ajout rapide d'item, composant `RevisionItemModal` (composition d'item typé + choix/création du jeu cible) et `TypedStudyCard` (étude interactive des items typés : révéler + feedback, auto-évaluation SM-2). Tests backend + vue-tsc/Vitest verts. |
| 2026-06-15 | Flashcard avancée (A1) | Mode inversé + tuning + courbe d'apprentissage | Remappée sur `Deck`/`Flashcard` (D3c). `Deck.reversed` (D7) + `tuning_default` (D4) ; `Flashcard.tuning` + `reverse_of_id`. Mode inversé matérialisé (carte miroir verso→recto, SM-2 distinct, étudiée séparément, exclue des listes/compteurs ; bascule via PUT deck) ; SM-2 × (tuning deck × tuning carte) ; courbe d'apprentissage `GET /decks/:id/cards/:card_id/history`. Frontend : toggles/sliders dans `Decks.vue`, badge « Inversé », composant `LearningCurve.vue` (SVG sans dépendance). Migration additive `c3d4e5f6a7b8`. Tests : backend 209/209, vue-tsc clean, Vitest 26/26, drift OK. |
| 2026-06-15 | Stats par élément (A7) | Indicateurs D5 (FSRS/Ebbinghaus/leeches) | Stats actionnables des éléments de révision (qcm/vf/assoc/def/ordre) depuis `StudySession.item_id`. Backend `revision_stats_service` : difficulté D, récupérabilité R (Ebbinghaus), stabilité S, rétention réelle (True Retention), sangsues, maturité, date de maîtrise (projection SM-2). `GET /stats/items/:id` + `GET /stats/sets/:id` (agrégats + verdicts + résumé par item, anti-N+1). Frontend : **`RevisionSetStats.vue`** (`/revision/sets/:id/stats`) KPI + verdicts + items + mini-courbe SVG ; bouton stats dans `Binders.vue`. Pas de migration. Tests : backend 219/219, vue-tsc clean, Vitest 28/28. |
| 2026-06-15 | Stats par classeur (A8) | Agrégat révision d'un classeur + sous-arbre | Clôt la Partie A. `GET /stats/binders/<uuid>` (option `?descendants=false`) agrège tous les ensembles de révision d'un classeur et de ses descendants : maîtrise, rétention réelle, réussite, sangsues, à-réviser, **répartition par type**, **ensembles à surveiller** + liste. Backend : refactor `RevisionStatsService` (`_aggregate_set` partagé), accès via `check_binder_access` (propriétaire ou classe partagée), agrégats bornés anti-N+1 (1 req. sets + 1 req. items + 1 req. sessions/type). Frontend : **`RevisionBinderStats.vue`** (`/revision/binders/:id/stats`) + bouton « Stats » dans `Binders.vue`. Périmètre : ensembles de révision (les decks flashcards gardent `/stats/decks/:id`). Pas de migration. Tests : backend 229/229 (dont `test_stats_binder`), vue-tsc clean, Vitest 33/33. |
| 2026-06-15 | Gestion de classeur (C1) | Rattacher/détacher des éléments existants | Le classeur devient gérable : on y déplace des éléments **déjà existants** (sans copie ni suppression). Backend : `BinderItemsService` + `POST /binders/:id/items` (attache) et `POST /binders/:id/items/detach` (détache) pour note/deck/ensemble/diagramme/pdf ; vérifs d'appartenance de l'élément **et** du classeur (`check_binder_access`). Frontend : modale **« Ajouter un élément existant »** (sélecteur multi : notes, jeux de révision, ensembles non rangés / d'un autre classeur) + bouton **« détacher »** par ligne (distinct de la suppression) dans `Binders.vue` ; store `attachItems`/`detachItems`. Reste : affichage des diagrammes/pdfs dans le classeur (backend déjà compatible). Pas de migration. Tests : backend 234/234 (`test_binder_items`), vue-tsc clean, Vitest 35/35. |
| 2026-06-15 | Onglet Révisions réorganisé (C2) | Catégories Classiques / IA | `Reviews.vue` passe d'onglets à plat à **deux catégories**. **Classiques** : sous-onglets Flashcards + QCM / Vrai-Faux / Association / Définition / Ordre — chaque type liste ses ensembles avec **étude/lancement**, **stats** (A7) et **réglages** (renommer, fine-tuning `tuning_default`, rattacher à un classeur, supprimer). **IA** : regroupe évaluation IA, feuille blanche, méthode Feynman, auto-QCM (panneaux existants). Store `updateSet` accepte `binder_id`. Frontend uniquement, pas de migration. Tests : vue-tsc clean, Vitest 36/36, E2E `/reviews` OK. |
| 2026-06-15 | Devoirs sur ensembles de révision (B3) | Type de tâche `revision` | Un devoir peut cibler un **ensemble de révision typé** (qcm/vf/association/définition/ordre). Backend : `TASK_TYPES` + `TASK_TARGET_KIND` étendus (`revision` → `revision_set`), `_resolve_task_target` vérifie l'appartenance au prof, complétion **dérivée** des `StudySession` des items de l'ensemble (`_revision_state`, objectif `{min_items, min_score}`). Frontend : `AssignmentBuilder` propose la cible « ensemble de révision » + objectif ; `TeacherDashboard` fournit la liste ; lancement élève via `taskLaunchRoute` → `/revision/sets/:id/study` (les QCM sont redirigés vers leur passage scoré) ; CTA « Réviser » dans `StudentClassView`. Pas de migration (`task_type` est un `String`). Tests : backend 237/237 (`test_assignment_revision`), vue-tsc clean, Vitest 37/37. |
| 2026-06-15 | Questions des élèves (B4) | Q&A classe + notifications | Un élève pose une question au professeur, qui y répond. Backend : modèle **`ClassQuestion`** (réponse unique, statut `open`/`answered`) + migration idempotente `a7c1d2e3f4b5` ; `ClassQAService` + endpoints `GET/POST /classes/:id/questions` et `POST /classes/:id/questions/:qid/answer`. Isolation : l'élève ne voit que ses questions, le professeur toutes celles de sa classe ; seul un prof peut répondre. **Notifications** in-app : type `question` aux professeurs (nouvelle question), type `answer` à l'auteur (réponse reçue). Frontend : section « Poser une question » (sélecteur de classe + fil) dans `StudentClassView`, onglet **« Questions »** (inbox + réponse inline) dans `TeacherDashboard`. Tests : backend 242/242 (`test_class_qa`), vue-tsc clean, Vitest 40/40. |
| 2026-06-15 | Stats de groupe étendues (B5) | Temps de révision, réussite, avancement par élève | Le tableau de bord prof gagne des métriques de révision. Backend : `get_class_overview` calcule le **temps moyen de révision** (`StudySession.duration_seconds`), le **taux de réussite global** (cartes correctes/révisées) et l'**avancement par élève** (`StudentStatSchema` : devoirs terminés, score moyen, minutes, réussite), en **agrégats SQL bornés** (budget de requêtes inchangé). Frontend : KPI « Révision moy. » et « Réussite révisions » + **tableau par élève** dans l'onglet Tableau de bord de `TeacherDashboard`. Pas de migration. Tests : backend 242/242 (`test_class_overview_revision_metrics` + budget), vue-tsc clean, Vitest 40/40. |
| 2026-06-15 | Révision active des notes (A9) | Cohérence du balisage + doc utilisateur | Consolidation du mode « Révision Active » des notes (balises inline, **sans persistance** : alimente les évals IA mais ne crée aucune flashcard/ensemble). **Incohérence corrigée** : l'aide intégrée de `NoteEdit.vue` documentait `ordre`/`assoc`/`vf` avec de mauvais séparateurs alors que le parser (`placeholder_parser`, `test_placeholders`), le rendu interactif et les boutons d'insertion utilisent `>` (ordre), `clé=valeur \| …` (assoc) et `::` (vf) ; aide alignée. Doc utilisateur ajoutée : `docs/revision-active-notes.md`. Pas de migration (texte d'aide + doc). Tests : `test_placeholders` OK, vue-tsc clean. **Dernier item : plan de refonte entièrement terminé.** |
| 2026-06-15 | Création des révisions depuis Révisions | Séparation création / organisation | La **création** des éléments de révision (carte/deck, QCM, V/F, définition, ordre, association) est déplacée du menu « + Ajouter » du **classeur** vers le menu **Révisions** : chaque onglet Classiques (Flashcards + types) a un bouton **« Créer »/« Nouvelle carte »** ouvrant `RevisionItemModal` (élément créé non rangé, `binderId=null`). Le classeur ne sert plus qu'à **organiser** : sous-dossiers, notes, et **rattacher des éléments existants**. Retrait dans `Binders.vue` des entrées de création du menu + boutons « Item »/« Ajouter un item ». Frontend uniquement, pas de migration. Tests : vue-tsc clean, Vitest 50/50. |
| 2026-06-15 | Upload PDF réel | Import / lecture / suppression / renommage de vrais PDF | Le module PDF passe du **simulé au réel** : le front s'appuie désormais sur l'API `/pdfs` existante au lieu de données factices. Frontend : `stores/pdf.ts` réécrit (`fetchPdfs`, `uploadPdf` multipart, `renamePdf`, `removePdf`, `openPdf` qui charge le fichier en **blob** via l'instance Axios authentifiée puis l'affiche dans une `<iframe>`), `PDFs.vue` (liste réelle, import, suppression, modale renommer + tags réels, badge « Cours partagé » en lecture seule), `PdfReader.vue` réécrit en visionneuse iframe. Les annotations simulées (sans backend) sont retirées. Backend : ajout de `PUT /pdfs/:id` (renommage, `PDFService.update_pdf`, écriture refusée sur un cours partagé). Tests : backend 250/250 (`test_pdfs` : upload/list/rename/file/delete + rejet non-PDF), `pdf.spec` (store), vue-tsc clean, Vitest 50/50. Pas de migration. |
| 2026-06-15 | Diagrammes & PDF dans le classeur (C1) | Affichage complet des contenus du classeur | Complète C1 : la vue classeur (`Binders.vue`) affiche désormais **tous** les types de contenu via des sections **Diagrammes** et **Documents PDF** (lecture `/diagrams?binder_id` & `/pdfs?binder_id`, navigation vers les pages dédiées). Les types `diagram`/`pdf` sont aussi ajoutés à la modale « Ajouter un élément existant » et au détachement par ligne (id diagramme = entier, id PDF = uuid). Frontend uniquement (backend `BinderItemsService` déjà compatible), pas de migration. Tests : `binders.spec` (attach diagram/pdf), vue-tsc clean, Vitest 45/45. |
| 2026-06-15 | Dépôt de cours (B1) | Le prof dépose une note/PDF visible de la classe | Un prof dépose un « cours » dans un **classeur de cours** partagé par référence à la classe (réutilise `GroupBinder`, pas de nouvelle entité). Backend : `POST /classes/:id/course-binder` (`ClassManagementService.get_or_create_course_binder`, idempotent, `CourseBinderSchema`) ; **partage PDF en lecture seule** étendu en miroir des notes (`pdf_service` surface les PDF des classeurs partagés `read_only`, accès via `check_binder_access`, suppression refusée à l'élève ; `PDFResponse.read_only`). Frontend : bouton **« Déposer un cours »** + modale (note) dans l'onglet Cours & Classeurs de `TeacherDashboard` ; `classService.getCourseBinder`. Upload PDF reporté (front PDF encore simulé, backend prêt). Pas de migration. Tests : backend 248/248 (`test_teacher_courses`), vue-tsc clean, Vitest 44/44. **Clôt la Partie B et le plan de refonte Révision/Professeur/UI.** |
| 2026-06-15 | Partage de classeur auto-MAJ (B2) | Partage par référence à la classe + indicateur | Le partage `GroupBinder` **par référence** (auto-actualisé) est désormais bouclé côté plan. Backend : test prouvant qu'un élément (note + sous-classeur) ajouté **après** le partage est immédiatement visible de l'élève (lecture seule) sans ré-action prof ; endpoint `GET /groups/binders/:uuid/classes` (`group_service.get_classes_sharing_binder`, schéma `BinderClassRefSchema`) pour l'indicateur, avec isolation propriétaire. Frontend : modale **« Partager ce classeur à une classe »** dans `Binders.vue` (classes animées, bascule partager/retirer) + badge **« Partagé (n) »** ; `groupService.getBinderClasses`. Pas de migration. Tests : backend 244/244 (`test_element_added_after_share_is_immediately_visible`, `test_binder_classes_indicator`), vue-tsc clean, Vitest 43/43. |
| 2026-06-15 | Étude typée (A3–A6) | Vrai/Faux, Association, Définition, Ordre | Étude + correction + SM-2 des types restants (socle révision). Backend : `check_answer` (VF verdict, Association appariement ordre-indifférent, Ordre strict) + `POST /revision/sets/:id/study/grade/:item_id` (SM-2 réussi→5/raté→2, `StudySession`) ; Définition en auto-évaluation via `answer`. Frontend : **`RevisionStudy.vue`** (`/revision/sets/:id/study`) — VF (verdict+justif), Définition (révéler+auto-éval), Association (menus d'appariement), Ordre (réordonnancement ▲▼) ; `gradeItem` au store ; `Binders.vue` lance l'étude des ensembles non-QCM. Pas de migration. Tests : backend 212/212, vue-tsc clean, Vitest 27/27. |
| 2026-06-15 | QCM scoré (A2) | Passage pondéré + réponses multiples | Sur le socle de révision (type `qcm`). `POST /revision/sets/:id/run` : correction pondérée par `points`, réponses multiples en **tout-ou-rien**, score/total/% + détail par question, mise à jour **SM-2 par question** (réussi→5, raté→1) + `StudySession` unifiée. Frontend : champ barème dans `RevisionItemModal`, store `runQcm`/`fetchSet`, vue **`QcmRun.vue`** (passage interactif + feedback), section « Ensembles de révision » dans `Binders.vue`. Pas de migration. Tests : backend 207/207, vue-tsc clean, Vitest 26/26. |
| 2026-06-15 | Socle de révision (D3c) | Ensembles typés indépendants + réconciliation PR #48 | Refonte du socle révision (cf. `REFACTO_FONCTIONNALITES.md` A0). **D3c** : le `Deck`/`Flashcard` redevient strictement recto/verso ; les autres types (QCM/VF/association/définition/ordre) sont des entités indépendantes → modèles **`RevisionSet`** (homogène, typé, `tuning_default`) + **`RevisionItem`** (payload validé par type, SM-2 + `tuning` par item). API `/api/v1/revision` (CRUD ensembles/items, étude, réponse SM-2). SM-2 enrichi d'un `tuning` (réviser plus/moins souvent). `StudySession.item_id`/`item_type` (suivi unifié, base stats D5). Migrations additive + réconciliation (déplace les cartes typées de la PR #48 vers des ensembles homogènes, idempotente). Frontend : store `revision.ts`, `RevisionItemModal` réconcilié (basic→deck, typés→ensembles), `StudyDeck` recto/verso seul (`TypedStudyCard` retiré, à réintroduire en A2–A6). Tests : backend 203/203, vue-tsc clean, Vitest 26/26. |
