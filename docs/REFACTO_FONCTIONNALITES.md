...
**État actuel** : ✅ **Fait** (PR `docs/revision-active-notes`, 2026-06-15). Mode « Révision Active » conservé ; cohérence vérifiée et **aide in-app corrigée** ; doc utilisateur ajoutée.

**Étapes** : [x] vérifier la cohérence après A0 — le balisage reste un self-test inline **sans persistance** (alimente les évals IA via `placeholder_parser`, ne crée aucune flashcard/ensemble ; cf. `no-auto-flashcards-from-notes`). **Incohérence trouvée & corrigée** : l'aide intégrée de `NoteEdit.vue` documentait `ordre`/`assoc`/`vf` avec de mauvais séparateurs (`|`/`:`) alors que le parser, le rendu interactif et les boutons d'insertion utilisent `>` (ordre), `clé=valeur | …` (assoc) et `::` (vf). · [x] doc utilisateur : `docs/revision-active-notes.md` (syntaxe de référence + principe « sans persistance »).
**Tests** : non-régression du parser de balises (`test_placeholders`, OK).
**Commits** : `docs(revision): clarifier la révision active des notes`. **PR** : `docs/revision-active-notes`.
...
**État actuel** : ✅ **Fait** (PR `feature/teacher-courses`, 2026-06-15). **Dernier item du plan.**

**Étapes**
- [x] (décision) « cours » = note/PDF rangés dans un **classeur de cours** partagé par référence à la classe (réutilise `GroupBinder`, **pas** de nouvelle entité).
- [x] (backend) Endpoint prof `POST /classes/:id/course-binder` (`ClassManagementService.get_or_create_course_binder`, `CourseBinderSchema`) : résout (idempotent) ou crée+partage le classeur de cours « Cours — {classe} ». Visibilité élèves en **lecture seule** déjà gérée pour les notes ; **étendue aux PDF** (miroir de `note_service` : `pdf_service` surface les PDF des classeurs partagés `read_only`, accès via `check_binder_access`, suppression refusée à l'élève).
- [x] (frontend) Espace prof : bouton **« Déposer un cours »** (onglet Cours & Classeurs de `TeacherDashboard`) → modale titre/contenu créant une **note** dans le classeur de cours (`classService.getCourseBinder` + `notesStore.createNote`). _Le « cours » prof = une **note** (décision dev). L'upload PDF réel a depuis été branché comme feature à part (cf. journal « Upload PDF réel »), les PDF déposés dans un classeur de cours partagé sont donc bien visibles des élèves._

**Tests** : backend `test_teacher_courses` (course-binder créé + idempotent, isolation prof → 403, note déposée visible élève `read_only`, PDF déposé visible `read_only` + suppression élève refusée + non-membre ne voit pas). Front `teacherCourses.spec`. Pas de migration.
...
**État actuel** : ✅ **Fait** (PR `feature/teacher-share-binder`, 2026-06-15). `GroupBinder` partage **par référence** ⇒ auto-update natif, désormais **vérifié par test** et **exposé depuis la vue classeur**.

**Étapes** : [x] (backend) vérifier que l'ajout d'éléments dans un classeur déjà partagé est immédiatement visible des élèves (test `test_element_added_after_share_is_immediately_visible` : note + sous-classeur ajoutés **après** partage → visibles côté élève, lecture seule). [x] (frontend) UI « Partager ce classeur à la classe » (modale dans `Binders.vue` listant les classes animées + bascule partager/retirer) + indicateur « partagé » (badge `Partagé (n)` alimenté par `GET /groups/binders/:uuid/classes`).
**Tests** : backend `test_element_added_after_share_is_immediately_visible` + `test_binder_classes_indicator` (liste classes, classeur non partagé = [], isolation propriétaire → 404). Front `binderClasses.spec` (getBinderClasses/shareBinder/unshareBinder). Pas de migration.
...
**État actuel** : ✅ **Fait** (PR `feature/teacher-group-stats`, 2026-06-15).
...
**État actuel** : ✅ **Fait** (PR `feature/binder-manage`, 2026-06-15 ; affichage diagrammes/PDF complété par `feature/binder-diagrams-pdfs`, 2026-06-15). Attache/détache existant ; visibilité ✅ ; renommer ✅ (update) ; **tous les types affichés** (notes, decks, ensembles, diagrammes, PDF).
...
- [x] (frontend) Modale « **Ajouter un élément existant** » (sélecteur multi) dans `Binders.vue` : notes, jeux de révision (decks), ensembles, **diagrammes, PDF** non rangés / d'un autre classeur.
- [x] (frontend) Retirer un élément du classeur (**détache**, ne supprime pas) — bouton dédié par ligne (notes, decks, ensembles, diagrammes, PDF), distinct de la suppression.
- [x] (frontend) Afficher **tous** les types dans le classeur : sections **Diagrammes** et **Documents PDF** (lecture `/diagrams?binder_id` & `/pdfs?binder_id`, navigation vers les pages dédiées).
- [x] (frontend) Renommer le classeur (via update) ; rappel visibilité public/privé (bouton Partager).

**Tests** : attache (note/deck/ensemble/**diagram/pdf**), détache conserve l'élément, rejet type inconnu (400), isolation (élément d'autrui, classeur d'autrui). Front : `binders.spec` (attachItems/detachItems, incl. diagram id entier & pdf id uuid).
**Commits** : `feat(binder): rattacher/détacher des éléments existants`, `feat(binder): affichage des diagrammes et PDF dans le classeur`.
**PR** : `feature/binder-manage`, `feature/binder-diagrams-pdfs`.
...
- **2026-06-15** — **B5 livré** (`feature/teacher-group-stats`). `get_class_overview` étendu : temps moyen de révision, taux de réussite global (cartes correctes/révisées) et **avancement par élève** (`StudentStatSchema`), en agrégats SQL bornés (budget de requêtes inchangé). Front : KPI « Révision moy. » + « Réussite révisions » + tableau par élève dans l'onglet Tableau de bord de `TeacherDashboard`. Pas de migration. Tests : backend 242 (`test_class_overview_revision_metrics` + budget), vue-tsc clean, Vitest 40. **Suite : B1 et B2 (sautés plus tôt) restent à livrer.**
- **2026-06-15** — **Correctif de suivi** : B5 avait été annoncé « dernier item / plan terminé » à tort. Relecture des cases : **B1** (déposer des cours) et **B2** (partage de classeur auto-MAJ) n'étaient **pas** livrés (sautés dans le séquencement, le journal passait de C2 à B3). Statut B5 corrigé (n'est pas le dernier item).
- **2026-06-15** — **B2 livré** (`feature/teacher-share-binder`). Le partage `GroupBinder` par référence était déjà fonctionnel (notes/decks/descendants remontent via `get_shared_root_binders`) ; cette PR **boucle les étapes du plan** : (1) test backend `test_element_added_after_share_is_immediately_visible` prouvant qu'un élément (note + sous-classeur) ajouté **après** le partage est immédiatement visible de l'élève, sans ré-action prof ; (2) endpoint `GET /groups/binders/:uuid/classes` (`group_service.get_classes_sharing_binder`, schéma `BinderClassRefSchema`) + test `test_binder_classes_indicator` (isolation propriétaire → 404, classeur non partagé = []). Front : modale « Partager ce classeur à une classe » dans `Binders.vue` (classes animées, bascule partager/retirer en lecture seule) + badge « Partagé (n) » ; `groupService.getBinderClasses`. Pas de migration. Tests : backend 244, vue-tsc clean, Vitest 43. **Suite : B1 (dépôt de cours) — dernier item.**
- **2026-06-15** — **B1 livré** (`feature/teacher-courses`). Dépôt de cours = note/PDF dans un **classeur de cours** partagé par référence (réutilise `GroupBinder`, décision actée). Backend : `POST /classes/:id/course-binder` (`get_or_create_course_binder`, idempotent, `CourseBinderSchema`) ; **partage PDF en lecture seule** étendu en miroir des notes (`pdf_service` : listing global inclut les PDF des classeurs partagés `read_only`, `_get_pdf_or_404` via `check_binder_access`, suppression réservée à l'écriture ; `PDFResponse.read_only`, `pdf_dao.get_by_binder_internal_ids`). Front : bouton « Déposer un cours » + modale (note) dans l'onglet Cours & Classeurs de `TeacherDashboard` ; `classService.getCourseBinder`. **Upload PDF reporté** (front PDF simulé, backend prêt). Pas de migration. Tests : backend 248 (`test_teacher_courses`), vue-tsc clean, Vitest 44. **Précision dev** : un « cours » côté prof = une **note** (pas un PDF) ; l'upload PDF réel front n'est pas un reliquat voulu. Reste : C1 (affichage diagrammes/pdfs), A9 (doc).
- **2026-06-15** — **C1 complété** (`feature/binder-diagrams-pdfs`). Affichage des **diagrammes** et **PDF** dans la vue classeur (`Binders.vue`) : sections dédiées (lecture `/diagrams?binder_id` & `/pdfs?binder_id`, filtrées par `binder_id`, navigation vers les pages dédiées), boutons **détacher** par ligne, et types **diagram/pdf** ajoutés à la modale « Ajouter un élément existant » (id diagramme = entier, id PDF = uuid). Frontend uniquement (backend déjà compatible), pas de migration. Tests : `binders.spec` (attach diagram/pdf), vue-tsc clean, Vitest 45. **Reste : A9 (vérif cohérence balisage + doc utilisateur).**
- **2026-06-15** — **A9 livré** (`docs/revision-active-notes`). Vérification de cohérence du balisage de notes après A0 : le mode « Révision Active » reste un self-test inline **sans persistance** (alimente les évals IA, ne crée aucune entité). **Incohérence trouvée et corrigée** : l'aide intégrée de `NoteEdit.vue` documentait `ordre`/`assoc`/`vf` avec de mauvais séparateurs (`|`/`:`) — alors que le parser (`placeholder_parser`, `test_placeholders`), le rendu interactif et les boutons d'insertion utilisent `>` (ordre), `clé=valeur | …` (assoc), `::` (vf). Aide alignée + doc utilisateur `docs/revision-active-notes.md` (syntaxe de référence). Pas de migration, pas de changement de logique (texte d'aide + doc). **🎉 Plan de refonte Révision/Professeur/UI entièrement terminé** (toutes les cases A/B/C cochées ; seul hors-plan : upload PDF réel front, non voulu — un « cours » prof = une note).
