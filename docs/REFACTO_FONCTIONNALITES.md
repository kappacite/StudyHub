...
- [x] (frontend) Espace prof : bouton **« Déposer un cours »** (onglet Cours & Classeurs de `TeacherDashboard`) → modale titre/contenu créant une **note** dans le classeur de cours (`classService.getCourseBinder` + `notesStore.createNote`). _Upload PDF reporté : le front PDF est actuellement simulé (pas d'upload réel) — le backend est prêt (partage PDF lecture seule)._
