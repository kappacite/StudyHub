<template>
  <div
    class="relative overflow-hidden bg-gradient-to-br from-primary via-primary to-accent text-white rounded-3xl p-6 shadow-xl shadow-elev-primary dark:shadow-none animate-fade-in group"
  >
    <!-- Background patterns for premium aesthetics -->
    <div
      class="absolute -right-10 -top-10 w-40 h-40 bg-surface/5 rounded-full blur-2xl group-hover:scale-125 transition-transform duration-500 pointer-events-none"
    ></div>
    <div
      class="absolute -left-10 -bottom-10 w-48 h-48 bg-accent rounded-full blur-3xl pointer-events-none"
    ></div>
    ...
    <div class="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
      ...
      <div class="space-y-4">
        <div>
          <span
            class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider bg-surface/10 backdrop-blur-md text-primary"
          >
        ...
        <div class="flex flex-wrap gap-2 text-xs font-bold uppercase tracking-wider">
          ...
          <span
            v-if="focusStore.assignmentCount > 0"
            class="px-2.5 py-1 rounded-xl bg-surface/5 text-primary border border-white/5"
          >
      ...
      <div
        class="flex flex-col items-center md:items-end gap-1.5 bg-surface/5 backdrop-blur-sm border border-white/10 rounded-2xl p-4 min-w-[160px]"
      >
        <span class="text-[9px] font-black text-primary uppercase tracking-widest"
          >Prévisions 7 jours</span
        >

        <!-- Inline SVG Sparkline -->
        <svg
          class="w-32 h-10 text-primary stroke-2 overflow-visible"
          viewBox="0 0 100 30"
          fill="none"
        >
          <!-- Sparkline Path -->
        ...
        <span class="text-[8px] text-primary font-semibold uppercase tracking-wider"
          >Total prévu : {{ forecastTotal }} cartes</span
        >
      </div>
...
onMounted(async () => {
  await Promise.all([
    ...
    // ensemble hétérogène (mode /study) : son échec ne doit pas bloquer le widget.
...
function startStudy() {
  ...
  if (!firstItem) return
  if (firstItem.type === 'assignment') {
    // Conserve la destination existante de ce widget (distincte de la
  ...
  router.push(getFocusItemTarget(firstItem, revisionStore.sets))
}
