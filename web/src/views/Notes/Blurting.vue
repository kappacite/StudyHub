        ...
        <div class="flex items-center gap-3">
          ...
          <div>
            <span class="text-[10px] font-bold text-primary uppercase tracking-wider"
              >Méthode de la page blanche (IA)</span
            >
        ...
        <div class="flex items-center gap-4">
          ...
          >
            <Clock
              ...
              :class="{ 'animate-pulse': step === 'writing' }"
            />
    ...
    <main class="flex-1 max-w-7xl mx-auto w-full p-6 flex flex-col">
      ...
      <div v-if="step === 'writing'" class="flex-1 flex flex-col gap-6">
        ...
        >
          ...
          <p class="text-xs text-ink-muted dark:text-ink-subtle mt-2 leading-relaxed">
            Fermez vos yeux, respirez un grand coup et essayez de rédiger ci-dessous tout ce dont
            vous vous souvenez de votre cours
            <strong>{{ noteTitle }}</strong
            >. Pas besoin de faire de belles phrases ni d'ordonner parfaitement vos propos. L'IA se
            chargera d'extraire et de cartographier vos connaissances, de repérer vos lacunes, et de
            générer des flashcards d'apprentissage ciblées.
          </p>
        </div>
        ...
        >
          ...
          >
            ...
            <div
              v-if="analyzing"
              class="text-primary flex flex-col items-end gap-1 text-right animate-pulse"
            >
              ...
              <span
                class="text-[10px] text-ink-subtle dark:text-ink-muted italic font-medium max-w-md transition-all duration-500"
              >
      ...
      <div v-else-if="step === 'results'" class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-6">
          ...
          >
            <h2 class="text-sm font-bold text-ink dark:text-ink-subtle uppercase tracking-wider">
              Cartographie des concepts du cours
            </h2>
            ...
            <div class="divide-y divide-line dark:divide-line">
              ...
              >
                ...
                <span class="mt-1 flex-shrink-0">
                  ...
                  <AlertTriangle
                    v-else-if="concept.status === 'incorrect'"
        ...
        <div class="space-y-6">
          <div
            class="bg-surface dark:bg-surface-soft border border-line dark:border-line rounded-3xl p-6 shadow-sm flex flex-col gap-6 sticky top-28"
          >
            <div class="space-y-2">
              <h2
                class="text-sm font-bold text-ink dark:text-white uppercase tracking-wider flex items-center gap-2"
              >
            ...
            <div class="space-y-3 max-h-[300px] overflow-y-auto pr-1">
              ...
              >
                ...
                <div class="text-xs space-y-1">
                  <div class="font-bold text-ink dark:text-ink-subtle">
                    Recto : {{ card.front }}
                  </div>
                  <div class="text-ink-muted dark:text-ink-muted italic">
                    Verso : {{ card.back }}
                  </div>
                </div>
              </div>
              ...
              <div
                v-if="resultData.suggested_flashcards.length === 0"
                class="text-center py-6 text-xs text-ink-subtle italic"
              >
            ...
            <div class="border-t border-line dark:border-line pt-4 space-y-4">
              <div class="space-y-2">
                <label class="text-xs font-bold text-ink-muted uppercase tracking-wider"
                  >Importer dans un deck</label
                >
...
onMounted(async () => {
  ...
  if (isFocusMode.value) {
    // Nécessaire uniquement pour la navigation "Continuer" de la file unifiée
    // (handleNextFocusItem) si l'item suivant est un ensemble de révision :
    // distingue le mode /run (QCM homogène) de /study. Son échec ne doit pas
    // bloquer la session de blurting en cours.
    try {
...
function handleNextFocusItem() {
  ...
  if (nextItem) {
    if (nextItem.type === 'note') {
      ...
      resetSession()
      loadNote()
    } else if (nextItem.type === 'deck' || nextItem.type === 'revision_set') {
...
async function submitForAnalysis() {
  ...
  try {
    ...
    const errMsg =
      err.response?.data?.error?.message || err.message || 'Une erreur inconnue est survenue.'
    alert(
      `Erreur d'analyse IA : ${errMsg}\n\nAssurez-vous que la clé d'API Gemini est correctement configurée et valide.`,
    )
...
async function createNewDeck() {
  ...
  try {
    const created = await decksStore.createDeck(
      newDeckName.value.trim(),
      `Deck créé depuis la page blanche de la note: ${noteTitle.value}`,
    )
...
async function importFlashcards() {
  ...
  const cardsToImport = resultData.value.suggested_flashcards.filter(
    (_, idx) => selectedCards.value[idx],
  )
