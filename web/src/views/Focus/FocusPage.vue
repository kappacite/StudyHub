<template>
  <div class="space-y-8 animate-fade-in">
    ...
    <div v-if="focusStore.loading" class="flex flex-col items-center justify-center py-20 gap-3">
      ...
      <span class="text-sm font-semibold text-ink-subtle uppercase tracking-widest">Chargement de votre espace focus...</span>
    </div>

    <div v-else-if="focusStore.error" class="p-6 bg-danger-soft border border-danger rounded-3xl dark:bg-danger-soft dark:border-danger text-danger dark:text-danger">
    ...
    <div v-else class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div class="space-y-4">
            ...
            >
              <div class="flex items-center gap-4 min-w-0">
                ...
                <div class="min-w-0">
                  <h4 class="font-bold text-sm text-ink dark:text-white truncate flex items-center gap-2">
                    {{ item.title }}
                    <span 
                      v-if="item.is_late"
              ...
              <div class="flex items-center gap-3">
                <span class="text-xs font-semibold text-ink-muted dark:text-ink-muted" v-if="item.last_session_ago_days !== null">
      ...
      <div class="lg:col-span-4 space-y-6">
        ...
        <div class="bg-surface dark:bg-surface-soft border border-line dark:border-line rounded-3xl p-6 shadow-sm">
          ...
          <div class="h-32 flex items-end justify-between gap-1 border-b border-line dark:border-line pb-1 relative mt-6">
            <div 
            ...
            >
              ...
              <span class="opacity-0 group-hover:opacity-100 transition-opacity bg-slate-950 dark:bg-surface-soft text-white dark:text-ink-subtle text-[8px] font-bold px-1.5 py-0.5 rounded -translate-y-5 z-10 whitespace-nowrap shadow absolute">
                {{ item.count }} cartes
              </span>
              <!-- Bar -->
              <div 
              ...
              ></div>
              <!-- Label -->
              <span class="text-[7px] font-black text-ink-subtle dark:text-ink-muted mt-1.5 rotate-45 origin-left whitespace-nowrap">
          ...
          <!-- Legend -->
          <div class="flex items-center justify-between mt-8 pt-4 border-t border-line-soft dark:border-line text-[10px] font-bold uppercase tracking-wider text-ink-subtle">
            <span class="flex items-center gap-1"><span class="w-2.5 h-2.5 rounded-sm bg-success border border-success"></span> Bas</span>
            <span class="flex items-center gap-1"><span class="w-2.5 h-2.5 rounded-sm bg-warning border border-warning"></span> Moyen</span>
            <span class="flex items-center gap-1"><span class="w-2.5 h-2.5 rounded-sm bg-danger border border-danger"></span> Fort</span>
          </div>
        </div>
        ...
        <div class="bg-surface dark:bg-surface-soft border border-line dark:border-line rounded-3xl p-6 shadow-sm">
          ...
          <div class="space-y-4">
            ...
            >
              <div class="flex items-center justify-between text-xs">
                ...
                <div class="flex items-center gap-2 font-semibold">
                  ...
                  <span 
                    ...
                    :class="sub.trend_7d > 0 ? 'text-success bg-success-soft dark:text-success dark:bg-success-soft' : 'text-danger bg-danger-soft dark:text-danger dark:bg-danger-soft'"
                  >
                    {{ sub.trend_7d > 0 ? '▲' : '▼' }} {{ Math.abs(sub.trend_7d) }}%
                  </span>
                </div>
              </div>
              <div class="w-full bg-surface-soft dark:bg-surface-soft rounded-full h-2 overflow-hidden flex">
                <div 
              ...
              <div class="flex items-center justify-between text-[9px] text-ink-subtle font-bold uppercase tracking-wider">
                <span>Rétention 30j</span>
                <span v-if="sub.overdue_count > 0" class="text-danger">{{ sub.overdue_count }} en attente</span>
                <span v-else class="text-success">À jour</span>
              </div>
            </div>

            <div v-if="focusStore.retention.length === 0" class="text-center py-4 text-ink-muted text-xs italic">
              Aucun classeur créé.
            </div>
          </div>
        </div>
