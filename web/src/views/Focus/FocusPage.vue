  ...
  <div class="space-y-8 animate-fade-in">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      <div>
        ...
        <p class="text-xs text-ink-subtle dark:text-ink-muted mt-1">
          Concentrez-vous sur l'essentiel : révisions prioritaires, prévisions de charge et taux de
          rétention.
        </p>
      </div>
      ...
      <button
        ...
        class="inline-flex items-center gap-2 px-5 py-2.5 border border-transparent rounded-xl text-sm font-semibold text-white bg-primary hover:bg-primary-strong active:scale-95 transition-all shadow-lg shadow-elev-primary disabled:opacity-50 disabled:cursor-not-allowed"
    ...
    <div v-if="focusStore.loading" class="flex flex-col items-center justify-center py-20 gap-3">
      ...
      <span class="text-sm font-semibold text-ink-subtle uppercase tracking-widest"
        >Chargement de votre espace focus...</span
      >
    </div>

    <div
      v-else-if="focusStore.error"
      class="p-6 bg-danger-soft border border-danger rounded-3xl dark:bg-danger-soft dark:border-danger text-danger dark:text-danger"
    >
    ...
    <div v-else class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
      ...
      <div class="lg:col-span-8 space-y-6">
        ...
        >
          <h3 class="font-bold text-ink dark:text-white flex items-center gap-2 mb-6">
          ...
          <div class="space-y-4">
            ...
            >
              <div class="flex items-center gap-4 min-w-0">
                ...
                <div class="min-w-0">
                  <h4
                    class="font-bold text-sm text-ink dark:text-white truncate flex items-center gap-2"
                  >
                    {{ item.title }}
                    <span
                      v-if="item.is_late"
                      class="px-2 py-0.5 rounded-lg text-[9px] font-bold uppercase tracking-wider bg-danger text-danger border border-danger"
                    >
                      En retard
                    </span>
                  </h4>
              ...
              <div class="flex items-center gap-3">
                <span
                  v-if="item.last_session_ago_days !== null"
                  class="text-xs font-semibold text-ink-muted dark:text-ink-muted"
                >
                  Dernière étude il y a {{ item.last_session_ago_days }}j
                </span>
                <button
                  class="px-4 py-2 border border-line dark:border-line rounded-xl text-xs font-bold text-ink hover:bg-surface-soft dark:text-ink-subtle dark:hover:bg-surface-soft transition-all active:scale-95 flex items-center gap-1.5"
            ...
            <div
              ...
              class="border-2 border-dashed border-line dark:border-line rounded-3xl p-12 flex flex-col items-center justify-center text-center text-ink-subtle"
            >
              <CheckCircle2 class="w-12 h-12 text-success mb-3" />
              <h4 class="font-bold text-ink dark:text-ink-subtle">Tout est à jour !</h4>
              <p class="text-xs mt-1">
                Vous n'avez aucune révision en attente. Profitez-en pour vous reposer ou explorer de
                nouveaux cours.
              </p>
            </div>
          </div>
      ...
      <div class="lg:col-span-4 space-y-6">
        ...
        <div
          class="bg-surface dark:bg-surface-soft border border-line dark:border-line rounded-3xl p-6 shadow-sm"
        >
          <h3 class="font-bold text-ink dark:text-white flex items-center gap-2 mb-4">
            <Calendar class="w-5 h-5 text-primary" />
            Charge à venir (14j)
          </h3>

          <div
            class="h-32 flex items-end justify-between gap-1 border-b border-line dark:border-line pb-1 relative mt-6"
          >
            <div
            ...
            >
              ...
              <span
                class="opacity-0 group-hover:opacity-100 transition-opacity bg-slate-950 dark:bg-surface-soft text-white dark:text-ink-subtle text-[8px] font-bold px-1.5 py-0.5 rounded -translate-y-5 z-10 whitespace-nowrap shadow absolute"
              >
                {{ item.count }} cartes
              </span>
              <!-- Bar -->
              ...
              <!-- Label -->
              <span
                class="text-[7px] font-black text-ink-subtle dark:text-ink-muted mt-1.5 rotate-45 origin-left whitespace-nowrap"
              >
          ...
          <!-- Legend -->
          <div
            class="flex items-center justify-between mt-8 pt-4 border-t border-line-soft dark:border-line text-[10px] font-bold uppercase tracking-wider text-ink-subtle"
          >
            <span class="flex items-center gap-1"
              ><span class="w-2.5 h-2.5 rounded-sm bg-success border border-success"></span>
              Bas</span
            >
            <span class="flex items-center gap-1"
              ><span class="w-2.5 h-2.5 rounded-sm bg-warning border border-warning"></span>
              Moyen</span
            >
            <span class="flex items-center gap-1"
              ><span class="w-2.5 h-2.5 rounded-sm bg-danger border border-danger"></span>
              Fort</span
            >
          </div>
        </div>
        ...
        >
          ...
          <div class="space-y-4">
            <div v-for="sub in focusStore.retention" :key="sub.binder_id" class="space-y-2">
              <div class="flex items-center justify-between text-xs">
                <span class="font-bold text-ink dark:text-ink-subtle">{{ sub.binder_name }}</span>
                <div class="flex items-center gap-2 font-semibold">
                  <span class="text-ink dark:text-white">{{ sub.retention_pct }}%</span>
                  ...
                  <span
                    ...
                    :class="
                      sub.trend_7d > 0
                        ? 'text-success bg-success-soft dark:text-success dark:bg-success-soft'
                        : 'text-danger bg-danger-soft dark:text-danger dark:bg-danger-soft'
                    "
                  >
              ...
              <div
                class="w-full bg-surface-soft dark:bg-surface-soft rounded-full h-2 overflow-hidden flex"
              >
                <div
              ...
              <div
                class="flex items-center justify-between text-[9px] text-ink-subtle font-bold uppercase tracking-wider"
              >
                <span>Rétention 30j</span>
                <span v-if="sub.overdue_count > 0" class="text-danger"
                  >{{ sub.overdue_count }} en attente</span
                >
                <span v-else class="text-success">À jour</span>
              </div>
            </div>

            <div
              v-if="focusStore.retention.length === 0"
              class="text-center py-4 text-ink-muted text-xs italic"
            >
              Aucun classeur créé.
            </div>
          </div>
...
onMounted(async () => {
  await Promise.all([
    ...
    // ensemble hétérogène (mode /study) : son échec ne doit pas vider le flux.
...
function getItemSummary(item: FocusItem) {
  ...
  if (item.type === 'revision_set') {
    return `${item.count} élément(s) de la série à revoir`
  }
...
function studyItem(item: FocusItem) {
  if (item.type === 'assignment') {
    // Conserve la destination existante de cet écran (distincte de la
    // référence Reviews.vue/bibliotheque) : hors périmètre de ce correctif,
    // qui ne traite que la branche `revision_set` manquante.
    router.push(`/binders/${item.id}`)
    return
  }
  if (item.type === 'deck' || item.type === 'note' || item.type === 'revision_set') {
