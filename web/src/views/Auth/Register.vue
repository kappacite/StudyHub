    ...
    <div v-motion="fadeUp" class="max-w-md w-full space-y-8 relative z-10">
      ...
      <BaseCard padding="lg" class="backdrop-blur-md">
        <form class="space-y-6" @submit.prevent="onSubmit">
          ...
          <div class="space-y-4">
            <!-- Username -->
            <BaseField label="Nom d'utilisateur" for-id="username">
              <BaseInput id="username" type="text" v-model="username" placeholder="SuperEtudiant">
            ...
            <BaseField label="Mot de passe" for-id="password">
              <BaseInput id="password" type="password" v-model="password" placeholder="Minimum 6 caractères">
          ...
          <!-- Submit Button -->
          <BaseButton type="submit" block size="lg" :loading="loading">Créer mon compte</BaseButton>
        </form>
        ...
        <div class="mt-6 text-center">
          <p class="text-sm text-ink-muted">
            Déjà inscrit ?
            <router-link to="/login" class="font-semibold text-primary hover:text-primary-strong transition-colors">
              Se connecter
            </router-link>
          </p>
        </div>
...
import { User, Mail, Lock, AlertCircle } from '@lucide/vue'
